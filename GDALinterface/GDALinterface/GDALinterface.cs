...
namespace GDALIPC
{
    public partial class GDALinterface : NamedPipeServer
    {
        private Guid _pingCommandId = Guid.Empty;
        ...
        public event EventHandler<EventArgs> HostReadyToProcess;
        public event EventHandler<CommandResult_Received_EventArgs> ListCommands_ResultReceived;
        public GDALinterface(string name, String hostPath, System.Action<string> log = null) : base(name, log)
        {
            ClientExecutablePath = hostPath;
            ServerStarted += GDALinterface_ServerStarted;
            Message_Received += GDALinterface_MessageReceived;
            Ping += GDALinterface_Ping; // Subscribe to the Ping event
            Pong += GDALinterface_Pong; // Subscribe to the Pong event
            HostReadyToProcess += GDALinterface_HostReadyToProcess; // Subscribe to the ClientReadyToProcess event
            Disconnected += GDALinterface_HostDisconnected; // Subscribe to the ClientDisconnected event
        }
        private async void GDALinterface_MessageReceived(object sender, Message_Received_EventArgs e)
        {
            try
            {
                // Deserialize the incoming message
                var message = JsonSerializer.Deserialize<MessageBase>(e.Message);
                Guid messageId = message.Id;  // Go ahead and grab the ID in case we need it
                ...
                switch (message.Type.ToLower())
                {
                    case "commandrequest": //first case, command request.  The server doesn't answer to many commands but we need to handle it anyways
                        ...
                        if (messageId == Guid.Empty)  //All command requests must have a GUID to map result responses back to them.  Exit if ID is null
                        {
                    ...
                    case "commandresponse":  //If the message is a command response, we need to generate some basic logging, invoke an event, and then ignore it.  The server doesn't respond to command responses.
                        ...
                        if (messageId == Guid.Empty)  //check if the command response has an ID.  If it doesn't, we can't map it back to a request, so ignore it. 
                        {
            ...
            catch (JsonException ex)
            {
                ...
                await Send(JsonSerializer.Serialize(new GenericMessage(Guid.Empty, $"Error: Invalid JSON: {ex.Message}")));
            }
            catch (Exception ex)
            {
                if (!(Log == null)) Log($"Error processing message: {ex.Message}");
                await Send(JsonSerializer.Serialize(new GenericMessage(Guid.Empty, $"Error: {ex.Message}")));
            }
        }
        private async void GDALinterface_Ping(object sender, CommandRequest_Received_EventArgs e)
        {
            if (!_pinged)
            {
                _pingCommandId = await SendCommandRequest("ping", null);
                _pinged = true; // Set the pinged flag to true
            }
        }
        private void GDALinterface_Pong(object sender, CommandResult_Received_EventArgs e)
        {
            if (_pinged)
            { 
                if (!_ponged)
                {
                    if (e.Message.Id == _pingCommandId)
                    {
                        ...
                        HostReadyToProcess?.Invoke(this, EventArgs.Empty); // Invoke the event to indicate the client is ready to process
                    }
                }
            }
        }
        private void GDALinterface_HostReadyToProcess(object sender, EventArgs e)
        {
            // Handle the event when the client is ready to process
            if (!(Log == null)) Log("GDAL host is ready to process commands.\r\n");
        }
        private async void GDALinterface_ServerStarted(object sender, EventArgs e)
        {
        ...
        private void GDALinterface_HostDisconnected(object sender, EventArgs e)
        {
            ...
            _pingCommandId = Guid.Empty; // Reset the ping command ID
